...
from bot.src.markups import close_appeal
    ...
    try:
        ...
        if await db.check_manager_active_appeal(call.from_user.id):
            ...
            if appeal_id:
                ...
                await bot.send_message(call.from_user.id, f"Вы приняли заявку:\n{text}", reply_markup=close_appeal(appeal_id))
                client_lang = await db.get_user_lang(appeal['user_id'])
                await bot.send_message(appeal['user_id'], "🤝 <b>Мутахассис ба дархости шумо пайваст шуд.</b> Шумо метавонед оғоз кунед ✨" if client_lang == "tj" else "🤝 <b>Специалист подключился к вашему запросу.</b> Можете начать общение ✨")
