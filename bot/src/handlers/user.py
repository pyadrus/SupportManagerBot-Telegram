...
async def choose_lang(call: CallbackQuery, state: FSMContext):
    try:
        ...
        await call.message.edit_text("Чӣ тавр ман метавонам кӯмак кунам, сайёҳ? ✨" if lang == 'tj' else "Чем могу помочь, путник? ✨", reply_markup=mk.start(lang))
...
async def start_create_appeal(call: CallbackQuery, state: FSMContext):
    try:
        ...
        if await db.check_user_active_appeal(call.from_user.id):
            await call.message.answer("💬 Шумо аллакай дар муколамаи фаъол ҳастед" if lang == 'tj' else "💬 Вы уже находитесь в активном диалоге")
        else:
            await call.message.answer("📝 Пеш аз пайваст шудан бо мутахассис, лутфан номи пурраи худро ворид кунед" if lang == 'tj' else "📝 Прежде чем связаться со специалистом, введите ваше ФИО")
            await state.set_state(st.StartAppealStates.fio)
    except Exception as e:
        logger.error(f"Ошибка начала создания обращения: {e} - {call.from_user.id}")
        
@router.message(StateFilter(st.StartAppealStates.fio))
async def fio_appeal(message: Message, state: FSMContext):
    try:
        await state.update_data(fio=message.text)
        lang = await db.get_user_lang(message.chat.id)
        await message.answer("📞 Бузург! Акнун рақами телефони тамосатонро ворид кунед" if lang == 'tj' else "📞 Отлично! Теперь введите ваш контактный номер телефона")
        await state.set_state(st.StartAppealStates.phone)
    except Exception as e:
        logger.error(f"Ошибка ввода ФИО обращения: {e} - {message.chat.id}")
        
@router.message(StateFilter(st.StartAppealStates.phone))
async def phone_appeal(message: Message, state: FSMContext):
    try:
        await state.update_data(phone=message.text)
        lang = await db.get_user_lang(message.chat.id)
        await message.answer("❓ Савол е мушкилоти худро ба қадри имкон муфассал тавсиф кунед" if lang == 'tj' else "❓ Опишите ваш вопрос или проблему как можно подробнее")
        await state.set_state(st.StartAppealStates.question)
    except Exception as e:
        logger.error(f"Ошибка ввода номера телефона обращения: {e} - {message.chat.id}")
        
@router.message(StateFilter(st.StartAppealStates.question))
...
\nЕсли всё верно, подтвердите, иначе измените данные"""
        await message.answer(text, reply_markup=mk.consent_or_edit_my_appeal(lang))
    except Exception as e:
...
async def consent_appeal(call: CallbackQuery, state: FSMContext):
    try:
        ...
        await call.message.edit_text("✅ <b>Дархости шумо қабул шуд!</b> Мунтазир бошед, мутахассиси мо ба зудӣ бо шумо тамос мегирад. Мо кор мекунем, дар ҳоле ки шаҳр хоб аст... 🌙" if lang == 'tj' else "✅ <b>Ваша заявка принята!</b> Ожидайте, наш специалист скоро свяжется с вами. Мы работаем, пока город спит... 🌙")
...
<b>Вопрос:</b> <code>{data['question']}</code>
        """
        await bot.send_message(settings.GROUP_ID, text, reply_markup=mk.manage_appeal())
    except Exception as e:
...
async def start_edit_appeal(call: CallbackQuery, state: FSMContext):
    try:
        ...
        await call.message.answer("✍️️ Дар муомилоти шумо чӣ тағир дода мешавад?" if lang == "tj" else "✍️ Что изменить в вашем обращении?", reply_markup=mk.edit_my_appeal(lang))
        await state.set_state(st.StartAppealStates.edit_type)
    except Exception as e:
        logger.error(f"Ошибка начала изменения обращения: {e} - {call.from_user.id}")
        
@router.callback_query(StateFilter(st.StartAppealStates.edit_type), F.data.startswith('edit_appeal-'))
async def edit_type_appeal(call: CallbackQuery, state: FSMContext):
    try:
        ...
        lang = await db.get_user_lang(call.message.chat.id)
        if edit_type == 'fio':
            await call.message.edit_text("📛 Лутфан номи пурраи навро ворид кунед" if lang == "tj" else "📛 Пожалуйста, введите новое ФИО")
        elif edit_type == 'phone':
            await call.message.edit_text("📞 Лутфан рақами нави телефонро ворид кунед" if lang == "tj" else "📞 Пожалуйста, введите новый номер телефона")
        else:
            await call.message.edit_text("❓ Лутфан саволи навро нависед" if lang == "tj" else "❓ Пожалуйста, введите новый вопрос")
        await state.set_state(st.StartAppealStates.edit)
    except Exception as e:
        logger.error(f"Ошибка начала изменения обращения: {e} - {call.from_user.id}")
        
@router.message(StateFilter(st.StartAppealStates.edit))
