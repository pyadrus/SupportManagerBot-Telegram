    ...
    try:
        ...
        if is_active:
            ...
            await callback_query.message.answer(
                ...
                if lang == "tj"
                else "💬 Вы уже находитесь в активном диалоге"
            )
        else:
            await callback_query.message.answer(
                "📝 Пеш аз пайваст шудан бо мутахассис, лутфан номи пурраи худро ворид кунед"
                if lang == "tj"
                else "📝 Прежде чем связаться со специалистом, введите ваше ФИО"
            )
...
async def fio_appeal(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            "📞 Бузург! Акнун рақами телефони тамосатонро ворид кунед"
            if lang == "tj"
            else "📞 Отлично! Теперь введите ваш контактный номер телефона"
        )
...
async def phone_appeal(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.info(f"Язык пользователя {message.chat.id}: {lang}")
        await message.answer(
            "❓ Савол е мушкилоти худро ба қадри имкон муфассал тавсиф кунед"
            if lang == "tj"
            else "❓ Опишите ваш вопрос или проблему как можно подробнее"
        )
...
async def consent_appeal(callback_query: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        # appeal_id = "1"
        logger.debug(f"ID созданной записи: {appeal_id}")
        text = f"""
...
async def start_edit_appeal(callback_query: CallbackQuery, state: FSMContext):
    try:
        ...
        await callback_query.message.answer(
            (
                "✍️️ Дар муомилоти шумо чӣ тағир дода мешавад?"
                if lang == "tj"
                else "✍️ Что изменить в вашем обращении?"
            ),
...
@router.callback_query(
    StateFilter(StartAppealStates.edit_type), F.data.startswith("edit_appeal-")
)
    ...
    try:
        ...
        if edit_type == "fio":
            await callback_query.message.edit_text(
                ...
                if lang == "tj"
                else "📛 Пожалуйста, введите новое ФИО"
            )
        elif edit_type == "phone":
            await callback_query.message.edit_text(
                "📞 Лутфан рақами нави телефонро ворид кунед"
                if lang == "tj"
                else "📞 Пожалуйста, введите новый номер телефона"
            )
        else:
            await callback_query.message.edit_text(
                "❓ Лутфан саволи навро нависед"
                if lang == "tj"
                else "❓ Пожалуйста, введите новый вопрос"
            )
